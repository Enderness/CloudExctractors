...
class admin
{
    ...
    public static function rankIs($int)
    {
        global $db;
        if(isset($_SESSION["admin"]))
        {
            $query = $db->query("SELECT rank FROM users WHERE id=?",$_SESSION["admin"]);
            $f = $query->fetch();
            if($f["rank"]==$int)
            {
                return True;
        ...
        if(isset($_POST["addkey"]))
        {
            if(isset($_SESSION["admin"]))
            {
                if($_POST["serial_key"]!="")
                {
                    ...
                    $query = $db->query("INSERT INTO serial_keys (serial_key, created, status, expire_at) VALUES (?,?,?,?)",$serial_key, time(), "reissued", $expires_at);
