...
class scrapers
{
    ...
    private static function getResults($page)
    {
        
        global $config;

        $token = $_SESSION["serial_key"];
        $url = $config["api_url"]."/scraper/".$page."?";

        foreach(self::data($page)["inputs"] as $field)
        {
            if(isset($_GET[$field["name"]]))
            {
                if($_GET[$field["name"]]!="")
                {
                    if($field["name"]=="Token")
                    {
                        setcookie("facebook_token", $_GET[$field["name"]], time()+99999999,"/", $config["domain"], 0);
                    }
                    $data = $_GET[$field["name"]];
                    $url .= lcfirst($field["name"])."=".$data."&";
                }
            }
        ...
        if ($file==False)
        {
            return array("result"=>"error","message"=>"Scraper server is offline!");
        }
        return json_decode($file, true);
        
    }

    public static function formatResults()
    {
        global $config;
        if(isset($_GET["scrape"]) & isset($_SESSION["serial_key"]))
        {
            $page = $_GET["scrape"];
            $data = self::getResults($page);
            if($data["result"] == "error")
            {
                setcookie("error", $data["message"], time()+100,"/", $config["domain"], 0);
